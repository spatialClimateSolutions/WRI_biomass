...
dry_tons_to_acres <- dry_tons_to_hectares %>%
  ...
  add_row(name = 'corn', landUseFactor = 9.3482) %>% # Add researched value here
  add_row(name = 'Soybean oil', landUseFactor = 10.06) %>% # Add researched value here
...
biomass_downscaling <- function(scenario_list){
  for (scenario in scenario_list){
    ...
    # Set null values to zero for numeric type conversion
    ...
    final_county <- county_level_results %>%
      left_join(zone_mapped %>% select(fips, sqmi), by = 'fips') %>% 
      left_join(dry_tons_to_acres, by = 'name') %>% # Add ton to acre conversion column
      mutate(production_acres = tons_of_production / landUseFactor) # Calculate acreage

    
    ######## Organize rows before writing output
...
# All possible scenarios: 'ira', 'central', 'slow.consumer.uptake', 'central.restrict', 'slow.consumer.uptake.restrict', 'central.COC', 'slow.consumer.uptake.COC'
scenarios <- c('ira', 'central', 'slow.consumer.uptake', 'central.restrict', 'slow.consumer.uptake.restrict', 'central.COC', 'slow.consumer.uptake.COC')
