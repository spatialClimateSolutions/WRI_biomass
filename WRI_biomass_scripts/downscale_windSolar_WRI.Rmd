...
########################################
# STEP 1. INPUT PARAMETERS AND SET FOLDER PATHS
...
outputsFolder_shp <- "../outputs/shp_011024/" ## if we need to write this output locally for plotting in ArcGIS
outputsFolder <- file.path(google_fp, "Downscaling_windSolar/downscaling_outputs/csvs_011024/")
outputsFolder_shp <- file.path(google_fp, "Downscaling_windSolar/downscaling_outputs/shp_011024/")
...
########################################
# STEP 2: READ prediction surfaces, RIO bins csv, RIO results csv, merged CPA as shapefile and raster

```{r}
#######################
# IMPORT PREDICTION SURFACES

## ******* PLACEHOLDER for prediction surface file paths
...
## **** PLACEHOLDER: need to reproject predPV and predWind to same projection as CPAs_ras

## Get df of prediction scores by CPA - PV
...
## **** PLACEHOLDER: make sure there's a UNIQUEID field in the resulting df, if not, rename the zone column

```


########################################
# STEP 3b: join prediction scores to RIO binned df

```{r}
######################################
## Join prediction scores df to RIO bins df
...
## join the prediction values to RIO bins 
rio_bins <- rio_bins %>% 
...
## drop CPA rows that have no prediction values
rio_bins_pred <- rio_bins %>% 
  drop_na(rf_PV)

print(paste("Number of CPAs missing prediction values:", toString(nrow(rio_bins) - nrow(rio_bins_pred))))

## delete objects that are no longer needed and take up a lot of memory
rm(rio_bins, pred_PV, pred_wind)
```


########################################
# STEP 4: Prep the technology, zone, scenario, bin lists for looping and subset the RIO capacity results df
```{r}
...
#tech.list.select <- c("wind+tracking pv","onshore wind", "fixed pv", "tracking pv")

# postprocess RIO capacity results to filter by technologies we're downscaling and isolate the bins from the technology (which are in the same column)
...
hold over from last script: CREATE NEW FIELDS FOR EACH SCENARIO AND YEAR COMBINATION IN CPA FC WITH VALUE = 1 IF CPA IS SELECTED
```{r}
