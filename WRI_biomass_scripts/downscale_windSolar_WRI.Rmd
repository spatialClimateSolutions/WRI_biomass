...
date <- "102125"
supplyScen <- "restricted" ## either "base" or "restricted" depending on the RIO scenario 
...
popNat_google_fp <- "/Users/grace/Library/CloudStorage/GoogleDrive-gracecwu@ucsb.edu/Shared drives/PoP_National/PoPNational_Data/0_Deliverables/multiTech_supplyCurve/Sept2023_updated" # <-- for the CPA shapefile locations
rioResultsFolder_fp <- file.path(google_fp, "RIO_data/FinalResults_June25")
rioResultsFolder_preliminaryResults_fp <- file.path(google_fp, "RIO_data/PreliminaryResults2")
...
## outputs folders:
outputsFolder <- file.path(google_fp, paste0("Downscaling_windSolar/downscaling_outputs/results_", date, "_", "NEWpredSurfaces" , "_", supplyScen))
...
pred_PV <- rast("/Users/grace/Documents/WRI_biomass/wind_solar_pred_surfaces_Sept2025/pred_randomForest_s.tif")
...
pred_wind <- rast("/Users/grace/Documents/WRI_biomass/wind_solar_pred_surfaces_Sept2025/pred_randomForest_w.tif")

#######################
# IMPORT RIO RESULTS
## Import RIO capacity results
rio_cap <- read.csv(file.path(rioResultsFolder_fp, "capacity.csv"))
head(rio_cap)
rio_tech_list <- unique(rio_cap$tech)

## Import RIO resource bins 
rio_bins <- read.csv(file.path(rioResultsFolder_preliminaryResults_fp, "binned_resource_df", paste0("binned_resource_df_", supplyScen,".csv")))
colnames(rio_bins)
...
scenario.list <- unique(rio_cap$run.name)
names(scenario.list) <- c("ira", "central", "slow", 
                          "centralRes", "slowRes", "centralCOC", 
                          "slowCOC", "cenResAll", "cenResDAC",
                          "slowResDAC", "cenResNo", "cenRes100",
                          "lowRes", "cenRes_v2")

if(supplyScen == "restricted"){
  scenario.list.short <- c("ira", "central", "slow consumer uptake", 
                           "central restrict", "slow consumer uptake restrict", "central COC",
                           "slow consumer uptake COC", "central restrict DAC",
                           "slow consumer uptake restrict DAC", "central restrict no store", "central restrict 100p re",
                           "low demand restrict", "central restrict v2")
  names(scenario.list.short) <- c("ira", "central", "slow", 
                          "centralRes", "slowRes", "centralCOC", 
                          "slowCOC", "cenResDAC",
                          "slowResDAC", "cenResNo", "cenRes100",
                          "lowRes", "cenRes_v2")
...
if(supplyScen == "base"){
  scenario.list.short <- c("central restrict all land")
  names(scenario.list.short) <- c("cenResAll")
}
