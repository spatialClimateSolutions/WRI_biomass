...
---
## Notes:
This script was modified from Henry's original "final_biomass_downscaling.Rmd" in order to run the updated "central restrict v2" scenario, include the "switchgrass-mar" feedstock which was unintentionally dropped in an earlier version of the biomassFeedstocksAndCategories_byScenario.csv, include  "existed mixed grasses - crp" which was also unintentionally dropped, and correct for the fact that data for these two feedstocks are only avaiilable for year 2025 (and thus applied to 2035 and 2050)
...
gdrive_path <- '/Users/grace/Library/CloudStorage/GoogleDrive-gracecwu@ucsb.edu/My Drive/Research_Projects/'
results_path <- 'WRI_biomass/RIO_data/FinalResults_June25/'

# Read in various supply related data
runs_key <- read.csv(file.path(gdrive_path, results_path, "runs_key.csv"))
fat_oil_grease <- read.csv(file.path(gdrive_path, results_path, 'fats_oil_grease.csv'))
potential_min_cropland_change <- read.csv(file.path(gdrive_path, results_path, 'potential_min_cropland_change.csv'))
biomass_feedstocks <- read.csv(file.path(gdrive_path, 'WRI_biomass/Downscaling_biomass/intermediateFiles/biomassFeedstocksAndCategories_byScenario_v3.csv'))
...
# Read and format weight-to-area data
 ## NEED TO REMEMBER TO CONVERT corn and soybean oil from metric tonnes to US tons per hectare
...
dry_tons_to_acres <- dry_tons_to_hectares %>%
  ...
  slice(7:24) %>% # remove unnecessary rows
  rename(name = Technology) %>%
  select(name, landUseFactor) %>% # Remove unit column
  #add_row(name = 'corn', landUseFactor = 10.2678) %>% # Researched value in US tons per hectare
  #add_row(name = 'soybean oil', landUseFactor = 0.6677) %>% # Researched value in US tons per hectare
  mutate(landUseFactor = landUseFactor/2.47105) # Convert from per hectare to per acre
...
demand_scenario_names <- c('ira' = 'ira', 'central' = 'central', 'slow.consumer.uptake' = 'slow consumer uptake',
                           ...
                           'central.restrict.v2' = 'central restrict v2')
...
biomass_downscaling <- function(scenario_list){
  for (scenario in scenario_list){
    ...
    filter(get(scenario) %in% c("ICCT x2 [d]", "BTS [d]", "min_cropland_change (llnl)", "3x expanded corn", "central restrict v2")) # Select current scenario in our three supply curve sources 
    ...
    ## modified Sept 3, 2025 to include "central restrict v2" as a filter criteria since it is a new llnl source for the central restrict v2 scenario
    llnl_scenario <- combined_supply %>% 
      filter(gau != 'missing', get(scenario) == 'min_cropland_change (llnl)' | get(scenario) == 'central restrict v2') %>% # Select LLNL data
      select(-scenario) %>% # Remove duplicate column
    ...
    ### figure out which feedstocks are missing data for 2050:
    feedstocks_2050 <- llnl_scenario %>% filter(year == "2050") %>% select(name) %>% unique()
    feedstocks_missing2050 <- unique(llnl_scenario$name)[!(unique(llnl_scenario$name) %in% unique(feedstocks_2050$name))]
    ...
    llnl_supply_prop <- left_join(llnl_scenario, llnl_zone_sum, by = c('name', 'year', 'bin', 'zone')) %>% # Add zone total data
      ...
      mutate(year = case_when( ## convert feedstocks that are missing year 2050 to 2050
          name %in% feedstocks_missing2050 ~ 2050,
          .default = year)) %>%
    ...
    ## import intermediate dataframe of commodity potential with the counties joined, generated by the demand_data_prep.rmd file 
    scenario_demand <- read.csv(file.path(gdrive_path, 'WRI_biomass/Downscaling_biomass/intermediateFiles/commodity_potential_county_level_v2.csv')) %>% 
      filter(run.name == demand_scenario_names[scenario]) # Extract demand data for current scenario
    ...
    if (dim(county_final_check[round(county_final_check$tons, 1) != round(county_final_check$tons_of_production, 1), ])[1] != 0){
      print('Warning, county and zone totals do not match')
        ## see which counties and zones don't match exactly
      View(county_final_check[round(county_final_check$tons, 1) != round(county_final_check$tons_of_production, 1), ])
    }
    ...
    if (dim(final_demand_difference[round(final_demand_difference$tons.x, 4) != round(final_demand_difference$tons.y, 4), ])[1] != 0){
      print('Warning: Final state demand was not accurately met')
      final_demand_difference[round(final_demand_difference$tons.x, 4) != round(final_demand_difference$tons.y, 4), ]
    }
    ...
    # Gather all feedstock-zone combinations where demand is still not being met
    incomplete_supply <- final_demand_difference[round(final_demand_difference$tons.x, 1) != round(final_demand_difference$tons.y, 1), ]
    ...
    if (dim(final_demand_difference[round(final_demand_difference$tons.x, 5) != round(final_demand_difference$tons.y, 5), ])[1] != 0){
      ...
      final_demand_difference[round(final_demand_difference$tons.x, 5) != round(final_demand_difference$tons.y, 5), ]
    }
    ...
    state_level_results <- state_level_results %>% left_join(biomass_feedstocks[c("category", "feedstock")], by = c("name" = "feedstock"))
    county_level_results <- county_level_results %>% left_join(biomass_feedstocks[c("category", "feedstock")], by = c("name" = "feedstock"))
...
# Select the scenarios for which you want to produce results
# All possible scenarios: 'ira', 'central', 'slow.consumer.uptake', 'central.restrict', 'slow.consumer.uptake.restrict', 'central.COC', 'slow.consumer.uptake.COC', 'central.restrict.all.land', 'central.restrict.DAC', 'slow.consumer.uptake.restrict.DAC', 'central.restrict.no.store', 'central.restrict.100p.re', 'low.demand.restrict', 'central.restrict.v2'

scenarios <- c('ira', 'central', 'slow.consumer.uptake', 'central.restrict', 'slow.consumer.uptake.restrict', 'central.COC', 'slow.consumer.uptake.COC', 'central.restrict.all.land', 'central.restrict.DAC', 'slow.consumer.uptake.restrict.DAC', 'central.restrict.no.store', 'central.restrict.100p.re', 'low.demand.restrict', 'central.restrict.v2')
