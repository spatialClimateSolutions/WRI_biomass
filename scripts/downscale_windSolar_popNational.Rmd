---
title: "Downscaling - PoP National"
author: "Grace wu"
date: "12/21/2022"
...
########################################
# STEP 1. INPUT PARAMETERS FOR EACH TECH
```{r}
...
## outputs folder:
outputsFolder <- "../../outputs/csvs_021023/"
outputsFolder_shp <- "../../outputs/shp_021023/"
outputsFolder_shp <- "/Users/grace/Library/CloudStorage/GoogleDrive-gracecwu@ucsb.edu/Shared drives/PoP_National/Data/DownscalingResults/shp_021023/"
...
########################################
# STEP 2: Import prediction values, bins csv, RIO results csv, merged CPA as shapefile
```{r}
rioFolderName <- "v3_021023"
...
## Import prediction values for each uniqueid cpa as df
pred_PV <- read.csv(paste0("../../../SupplyCurveAnalysis/attributeData/", zonalStats_folderName, "/", rf_field_PV , ".csv")) %>% 
  select(-X) %>%
  rename(rf_PV = UQ(rlang::sym(rf_field_PV)))
pred_wind <- read.csv(paste0("../../../SupplyCurveAnalysis/attributeData/", zonalStats_folderName, "/", rf_field_wind , ".csv")) %>% 
  select(-X) %>%
  rename(rf_wind = UQ(rlang::sym(rf_field_wind)))

## Import RIO capacity results
rio_cap <- read.csv(paste0("../../../RIOresults/", rioFolderName, "/capacity_y.csv"))
head(rio_cap)
...
## AND calculate the new field, CPA_APV2 using nonWheat, delete nonWheat
## AND join the SVI attributes using the zonal stats df
CPA_1 <- st_read(paste0("../../../SupplyCurveAnalysis/multiTechCPAs/", mergedCPAs_folderName, "/merged_CPA_focalStats_1_800000.shp"), stringsAsFactors = FALSE) #%>%
  #mutate(CPA_APV2 = nonWheat) %>%
  #select(-nonWheat)
CPA_2 <- st_read(paste0("../../../SupplyCurveAnalysis/multiTechCPAs/", mergedCPAs_folderName, "/merged_CPA_focalStats_800001_1600000.shp"), stringsAsFactors = FALSE) #%>%
  #mutate(CPA_APV2 = nonWheat)%>%
  #select(-nonWheat)
CPA_3 <- st_read(paste0("../../../SupplyCurveAnalysis/multiTechCPAs/", mergedCPAs_folderName, "/merged_CPA_focalStats_1600001_plus.shp"), stringsAsFactors = FALSE) #%>%
  #mutate(CPA_APV2 = nonWheat)%>%
  #select(-nonWheat)
